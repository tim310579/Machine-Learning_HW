...
print("Confusion Matrix with Holdout validation without Laplace smoothing")
print(outcome)
...
for y in range (0, len(df_test)):
    ...
    i = i + 1
#print(TP, FN, FP, TN)
outcome_l = pd.DataFrame(index = ['Actual Positive(etible)', 'Actual Negative(poison)'], columns = ['Predict Positive(etible)', 'Predict negative(poison)'])
outcome_l['Predict Positive(etible)'] = [TP, FP]
outcome_l['Predict negative(poison)'] = [FN, TN]
print("Confusion matrix with Holdout validation with Laplace smoothing--------")
print(outcome_l)
acc = (TP+TN) / (TP+TN+FP+FN)
rec = TP/(TP+FN)
pre = TP/(TP+FP)
...
print('')
print("Average Confusion matrix with K-fold validation without Laplace smoothing--")
...
#print("Precision:", pre_k2)
print('')
print("Average Confusion matrix with K-fold with Laplace smoothing------------")
outcome_avg = pd.DataFrame(index = ['Actual Positive(etible)', 'Actual Negative(poison)'], columns = ['Predict Positive(etible)', 'Predict negative(poison)'])
outcome_avg['Predict Positive(etible)'] = [(tp+tp1+tp2)/3, (fp+fp1+fp2)/3]
outcome_avg['Predict negative(poison)'] = [(fn+fn1+fn2)/3, (tn+tn1+tn2)/3]
print(outcome_avg)
